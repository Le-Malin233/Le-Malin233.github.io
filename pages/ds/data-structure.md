...
## 2026.3.26
### 树和森林
这一章主要关注一般树和森林的存储和处理，以及树、森林和二叉树的转换。

### 树的存储实现
如果按照二叉树的标准存储形式，每个数据存放在一个节点中，对于树来说效率非常低。这是因为每个节点要保存它所有儿子的地址，但是有些节点可能有很多儿子，有些节点可能没有儿子，因此我们需要其他更加实用的方法来存储一般的树。以下所有方法的示例都被用来描述下面这棵树：
```
      A
    / | \
   /  |  \
...
#### 邻接表
书上提了一个奇怪的名字叫孩子链表示法，实际上就是一种特殊的邻接表。**邻接表**是用来存储图的一种数据结构，在这里也同样适用，因为树本质上就是一种特殊的图，这在之前定义部分也有提到。

邻接表分为两部分，一部分是表头数组，用来存储所有节点的数据，另一部分是指针，指向每个节点的所有儿子。我们在这里可以把树理解为有向图，即每条边是由父节点指向子节点，这样就完整符合邻接表的定义了。下面是邻接表的示例，左边存储的是树的每个节点，右边是该节点的所有儿子。
```
A|->B->C->D
B|->E
C|->F->G
...
#### 孩子兄弟链表示法
孩子兄弟链表示法是用二叉树表示一般树的一种方法。在这种方法中，节点形式和二叉树形式是一致的，但是节点的左指针是指向它第一个儿子，右指针是指向它的兄弟。下面是孩子兄弟链表示法的示例。
```
    A
   /
  B
...
#### 双亲表示法
双亲表示法简单来说就是存储每个节点和它的父亲信息，直接看下面这个示例就能理解了：
```
编号 数据 父节点
---------------
 1 |  A    0
...
关于一般树的遍历，除了不便于中序遍历以外，与二叉树的遍历没什么区别，此处不再赘述。

---
### 优先级队列
优先级队列出队的顺序并不是入队的顺序，而需要根据元素的优先级决定。如果使用线性结构实现优先级队列，需要在入队和出队时对元素进行排序或查找，这需要 $O(n)$ 时间，但是如果使用树状结构实现，我们可以使出队和入队的时间复杂度降到 $O(\log n)$，这就是我们接下来要介绍的二叉堆。

#### 二叉堆
二叉堆是一颗满足**结构性**和**有序性**的二叉树，结构性要求二叉树最好是一棵满二叉树，至少也应该是一棵完全二叉树，这样能保证二叉堆的时间性能；有序性则要求最小（或最大）的元素位于根节点位置，每个节点的子树都要大于（或小于）这个节点本身。当根节点是最小元素时，称为**最小化堆**（小顶堆），反之称为**最大化堆**（大顶堆）。

二叉堆的入队和出队通过**向上过滤**和**向下过滤**实现。
- 向上过滤：以最小化堆为例，当一个新元素入队时，先将其放在完全二叉树的下一个可用位置，然后与父节点比较，如果比父节点小，就将其与父节点交换位置，重复这样的操作直到插入的这个元素大于父节点为止。如果这个元素是最小的元素，它最终会到达根节点的位置。
- 向下过滤：同样以最小化堆为例，当队列头出队时，弹出根节点的元素，然后把二叉堆中最后一个位置（不一定是最大）的元素放在根节点的位置。然后与两个儿子比较，如果大于两个儿子，选择一个较小的交换；如果只大于某一个儿子，就与该儿子交换，重复直到不能再与儿子交换为止。

上面的例子是最小化堆的情况，如果是最大化堆，只需要在比较的时候交换不等号的方向即可。因为树的深度是对数大小的，最坏情况下的时间复杂度是 $O(\log n)$，相比线性结构的$O(n)$ 时间还是更有优势的。

从一组数据构建二叉堆，首先需要将其按顺序填入完全二叉树中，然后从最后一个非叶子节点开始，依次向前对每个节点执行向下调整。对于小顶堆，父节点与较小的子节点比较，若父节点大于子节点则交换，继续向下调整；对于大顶堆，父节点与较大的子节点比较，若父节点小于子节点则交换，继续向下调整。所有调节完成后，即是一个符合条件的二叉堆。

<!-- pagebreak -->

<!-- ## 2026.4.10
>现在是4月10日晚上8点，下午刚过完原神月之六的世界任务，被美丽的风景和美好的故事深深打动，遂充满动力，重新开始继续这篇文章。
### D堆
D堆通过降低树的高度进一步提高性能，它的每个节点有D个儿子，当D大于2时，生成的堆比二叉堆矮，它将入队的时间复杂度降低为 $O(\log_D n)$。但是出队操作相对更加耗时，因为在向下过滤时，每个节点要找到最小儿子，故时间复杂度为 $O(D\log_D n)$。不过因为D是常数，总体而言的时间还是 $O(\log n)$。

### 归并优先级队列
二叉堆不能有效支持两个优先级队列的归并操作，不过下面将要提到的这些数据结构能有效支持 -->
